...
Btnvalue= readVoltage(Joystick_arduino,swPin);   
  ...
  switch mode 
    ...
    if(xvalue>2.4445 && wizard_c<max_dim)
        foreground(wizard_r,wizard_c)=1;
        wizard_c=wizard_c+1;
        foreground(wizard_r,wizard_c)=blue_guy_pos;
    elseif(xvalue<2.4430 && wizard_c>1)
        foreground(wizard_r,wizard_c)=1;
        wizard_c=wizard_c-1;
        foreground(wizard_r,wizard_c)=blue_guy_pos;
    elseif(yvalue<2.4275 && wizard_r>1)
        foreground(wizard_r,wizard_c)=1;
        wizard_r=wizard_r-1;
        foreground(wizard_r,wizard_c)=blue_guy_pos;
    elseif(yvalue>2.4790 && wizard_r<max_dim)
        foreground(wizard_r,wizard_c)=1;
        wizard_r=wizard_r+1;
       foreground(wizard_r,wizard_c)=blue_guy_pos;
    elseif(Btnvalue==0)
 ...
 %hi ben
    ...
    elseif(yvalue<2.4275 && wizard_r>1)
        ...
        port_y = port_y-1;
        background(port_y,port_x)=2;
    elseif(yvalue>2.4785 && wizard_r<max_dim)
         background(port_y,port_x)=6;
        port_y = port_y+1;
        background(port_y,port_x)=2;
     elseif(Btnvalue==0)
