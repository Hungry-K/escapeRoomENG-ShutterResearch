           ...
           switch pressedKey
               ...
               case 'rightarrow'
                   if player_xPosition < maxDimensionHorizontal
                    ...
                    player_xPosition = player_xPosition + 1; 
                    foreground(player_yPosition, player_xPosition) = playerSpriteValue;
                   end

               case 'leftarrow'
                   if player_xPosition > 1
                    foreground(player_yPosition, player_xPosition) = 1;
                    player_xPosition = player_xPosition - 1; 
                    foreground(player_yPosition, player_xPosition) = playerSpriteValue;
                   end
               ...
               case 'r'
                   selectedMode = 0;   
               otherwise 
                   disp ("not a registered input :(")
           end % ends switch 
               drawScene(room, background, foreground)
    end % ends Walking loop





    %start of portal loop
     while selectedMode == 0
           
           pressedKey = getKeyboardInput(room)
            disp ('this is portal mode')
           switch pressedKey
